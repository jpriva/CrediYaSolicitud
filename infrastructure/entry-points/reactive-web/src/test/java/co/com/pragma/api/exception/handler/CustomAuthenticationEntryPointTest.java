...
class CustomAuthenticationEntryPointTest {
    ...
    void setUp() {
        ...
        when(path.value()).thenReturn("/test/unauthorized");
        when(response.getHeaders()).thenReturn(headers);
        when(response.bufferFactory()).thenReturn(bufferFactory);
    }

    @Test
    @DisplayName("Should set 401 status and write JSON error body on authentication failure")
    void commence_shouldSetUnauthorizedStatusAndWriteErrorBody() throws JsonProcessingException {
        // Arrange
        AuthenticationException authException = new AuthenticationException("Test Auth Failed") {};
        ...
        // Verificar que se establecieron el estado y las cabeceras correctas
        verify(response).setStatusCode(HttpStatus.UNAUTHORIZED);
        verify(headers).setContentType(MediaType.APPLICATION_JSON);

        // Capturar y verificar el Publisher que se pasa a writeWith
        ...
        // Suscribirse al Publisher para desencadenar la lógica interna (la llamada a objectMapper)
        StepVerifier.create(publisherCaptor.getValue())
                .expectNext(dataBuffer)
                .verifyComplete();

        // Ahora que el Publisher se ha ejecutado, se puede verificar la serialización
        ...
        assertEquals(Errors.INVALID_CREDENTIALS_CODE, dtoCaptor.getValue().getCode());
        assertEquals("/test/unauthorized", dtoCaptor.getValue().getPath());
    }
}